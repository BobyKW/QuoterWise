...
const limitsFormSchema = z.object({
  ...
  anonymousBlockLimit: z.coerce.number().min(0, 'Must be a positive number.'),
  registeredBlockLimit: z.coerce.number().min(0, 'Must be a positive number.'),
});
...
export function QuoteLimitsForm() {
  ...
  return (
     <Card>
        <CardHeader>
          <CardTitle>{t('admin_page.app_limits_title')}</CardTitle>
          <CardDescription>{t('admin_page.app_limits_description')}</CardDescription>
        </CardHeader>
        <CardContent>
            <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4 rounded-lg border p-4">
                        <h3 className="font-medium">{t('quote_limits_form.quote_limits_heading')}</h3>
                        <FormField
                     ...
                     <div className="space-y-4 rounded-lg border p-4">
                        <h3 className="font-medium">{t('quote_limits_form.block_limits_heading')}</h3>
                        <FormField
                            control={form.control}
                            name="anonymousBlockLimit"
                            render={({ field }) => (
                            <FormItem>
                                <FormLabel>{t('quote_limits_form.anonymous_block_label')}</FormLabel>
                                <FormControl>
                                <Input type="number" {...field} />
                                </FormControl>
                                <FormMessage />
                        ...
                        <FormField
                            control={form.control}
                            name="registeredBlockLimit"
                            render={({ field }) => (
                            <FormItem>
                                <FormLabel>{t('quote_limits_form.registered_block_label')}</FormLabel>
                                <FormControl>
                                <Input type="number" {...field} />
                                </FormControl>
                                <FormMessage />
